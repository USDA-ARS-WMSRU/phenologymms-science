!  *********************************************************************
!  *                                                                   *
!  *                S U B R O U T I N E   P H E N O L W B	           *
!  *                                                      gm   1/30/05 *
!  *                                                                   *
!  *                                                                   *
!  *********************************************************************
!  This subroutine calculates phenological growth stages for 
!  winter barley.
...
!          srs(C,R), tis(C,R)
      ...
      subroutine phenolwb(aepa, aifs, antes, antss, boots, dae, dap, 
     ...
     c nolvs, pchron, srs, tis, year)
      ...
      integer  aifs(4), antes(4), antss(4), boots(4), dae, dap, dav,   
     c daynum, ddae(20), ddap(20), ddav(20), drs(4), first7, fps(4), 
     c heads(4), hrs(4), ies(4), joints(4), mats(4), srs(4), tis(4), 
     c year 
...
!  wheat, or after emergence for spring wheat and spring barley.
      ...
      elseif ((srs(1) .eq. 999) .and. (gddv .ge. dummy2(3))) then
          srs(1) = daynum
          srs(2) = year
          call date1(srs)
	     ddap(3) = dap
...
!  Awn Initials Formed stage:
      ...
      elseif ((aifs(1) .eq. 999) .and. (ies(1) .eq. 999) .and.  
     c  (gddv .ge. (dummy2(3) + dummy2(4) + dummy2(5)) )) then
          aifs(1) = daynum
          aifs(2) = year
          call date1(aifs)
	     ddap(5) = dap
...
!      elseif ((ies(1) .eq. 999) .and. (gddv .ge. (dummy2(3) + dummy2(4) + 
!     c  dummy2(5)) )) then
            ies(1) = daynum
            ies(2) = year
            call date1(ies)
...
!  Jointing growth stage prediction:
      ...
      elseif ((joints(1) .eq. 999) .and. (gddv .ge. (dummy2(3) 
...
!  completed its growth.  
      ...
      elseif ((boots(1) .eq. 999) .and. (gddv .ge. (dummy2(3) 
...
!  as heading if enough degree-days accumulated today.
      ...
      elseif ((heads(1) .eq. 999) .and. (gddv .ge. (dummy2(3)
...
!  accumulated today.
      ...
      elseif ((antss(1) .eq. 999) .and. (gddv .ge. (dummy2(3) 
     c   + dummy2(4) + dummy2(5) + dummy2(6) + dummy2(7)  
...
!  Physiological maturity:
      ...
      elseif ((mats(1) .eq. 999) .and. (gddv .ge. (dummy2(3) 
...
! Time to harvest ripe:
      ...
      elseif ((hrs(1) .eq. 999) .and. (gddv .ge. (dummy2(3) 
     c  + dummy2(4) + dummy2(5) + dummy2(6) + dummy2(7) + dummy2(8) 
...
!  double ridge.
     ...
     c     (gddv .ge. (dummy2(3) + dummy2(4) + (0.3*pchron)) )) then
              fps(1) = daynum
              fps(2) = year
              call date1(fps)
	         ddap(7) = dap
...
!  End of anthesis:
     ...
     c   (gddv .ge. (dummy2(3) + dummy2(4) + dummy2(5) + dummy2(6) 
